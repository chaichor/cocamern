...
const TransportPage = () => {
  ...
  const fetchTransport = async () => {
    try {
      ...
      const { data } = await axios.get('http://localhost:4000/api/transport', config);
      setTransport(data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        await axios.put(`http://localhost:4000/api/transport/${editId}`, formData, config);
      } else {
        await axios.post('http://localhost:4000/api/transport', formData, config);
      }
      ...
      alert('Error al guardar el transporte: ' + (error.response?.data?.message || error.message));
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar este transporte?')) {
      try {
        ...
        await axios.delete(`http://localhost:4000/api/transport/${id}`, config);
        fetchTransport();
      } catch (error) {
  ...
  const filteredTransport = Array.isArray(transport)
    ? transport.filter(t =>
        t.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        t.num?.toLowerCase().includes(searchTerm.toLowerCase())
      )
    : [];
  ...
  return (
    <div className="h-full w-full flex flex-col">
      <div className="w-full flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 sm:gap-0">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Gestión de transportes</h1>
        <button 
        ...
        >
          <Plus className="h-4 w-4 mr-2" /> Nuevo transporte
        </button>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-6 w-full overflow-hidden">
        ...
        {loading ? (
          <div className="text-center py-6 sm:py-8">
            ...
            <p className="mt-2 text-gray-600 text-sm sm:text-base">Cargando transportes...</p>
          </div>
        ) : filteredTransport.length === 0 ? (
          <div className="text-center py-6 sm:py-8">
            <p className="text-gray-600 text-sm sm:text-base">No se encontraron transportes.</p>
          </div>
        ) : (
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Número</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredTransport.map((transport) => (
                  <tr key={transport._id}>
                    <td className="px-3 sm:px-6 py-3 whitespace-nowrap font-medium text-gray-900 text-sm sm:text-base">
                      {transport.name}
                    </td>
                    <td className="px-3 sm:px-6 py-3 whitespace-nowrap text-sm text-gray-500">
                      {transport.num && transport.num.length > 50
                        ? `${transport.num.substring(0, 50)}...`
                        : transport.num}
                    </td>
          ...
          <div className="bg-white rounded-lg shadow-xl p-4 sm:p-6 w-full max-w-xs sm:max-w-md mx-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg sm:text-xl font-bold text-gray-800">
                {editId ? 'Editar transporte' : 'Nuevo transporte'}
              </h2>
              <button 
                onClick={resetForm}
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  <label htmlFor="num" className="block text-sm font-medium text-gray-700 mb-1">Número</label>
                  <textarea
                    id="num"
                    name="num"
                    rows="3"
